...
STORAGE_DIR = Path("./meetings_data")
STORAGE_DIR.mkdir(exist_ok=True)
meetings = {}  # meeting_id -> { "chunks": [paths], "chats": [...] }
...
#     # Concatenate using ffmpeg for proper audio merging
...
#     # ---- TRANSCRIPTION ----
#     transcript_text = await transcribe_with_whisper(concatenated)
...
async def finalize_meeting(meeting_id: str, title: Optional[str] = None):
    ...
    meeting = meetings[meeting_id]
    chunks = meeting.get("chunks", [])
    # chunks = sorted(chunks, key=lambda p: int(Path(p).name.split("_chunk_")[1].split(".")[0]))
    chats = meeting.get("chats", [])
    ...
    def ts(p):
        name = Path(p).name
        # you have guid_chunk_16923423423.webm
    ...
    # transcript_text = await transcribe_with_whisper(wav_path)
...
def prepare_audio_for_whisper(src_path: Path) -> Path:
...
async def transcribe_with_whisper(audio_path: Path) -> str:
    ...
    # safe_audio = prepare_audio_for_whisper(audio_path)
